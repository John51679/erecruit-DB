...
create database IF NOT EXISTS erecruit;
use erecruit;
...
PRIMARY KEY (title),
CONSTRAINT ANTIK_ANTIK FOREIGN KEY (belongs_to) REFERENCES antikeim(title) ON DELETE CASCADE ON UPDATE CASCADE
)engine=InnoDB;
...
/* add CANDIDATES */
insert into `user` (username, usr_password, first_name, surname, reg_date, email) values 
...
/* add ADMINISTRATOR */
insert into `user` (username, usr_password, first_name, surname, reg_date, email) values 
('vagos98', 'root', 'Evaggelos', 'Soulintzhs', '2019-06-23 17:12:34', 'soulintzis@ceid.gr'),
('giannis98', 'toor', 'Giannhs', 'Akarephs', '2019-07-23 18:21:43', 'akarepis@ceid.gr');
